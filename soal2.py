...
class Matriks:
    ...
    def __mul__(self, matriksLain):
        # Cek apakah banyak kolom object matriks sama dengan banyak baris
        # matriksLain.
        if self.bykKolom() != matriksLain.bykBaris():
            raise ValueError('Ukuran matriks tidak sesuai untuk perkalian matriks.')
        else:
            hasilMtrk = Matriks(self.bykBaris(), matriksLain.bykKolom())
            for brsA in range(self.bykBaris()):
                for klmB in range(matriksLain.bykKolom()):
                    for klmA in range(self.bykKolom()):
                        hasilMtrk[brsA, klmB] += self[brsA, klmA] * matriksLain[klmA, klmB]
            return hasilMtrk